        ...
        appBar: AppBar(
          ...
          actions: [
            IconButton(
              ...
              onPressed: () {
                Navigator.of(context).pushNamed(AppRoutes.parentSettings);
              },
            ),
          ],
        ),
      ...
      children: [
        ...
        if (children.isEmpty)
          ...
          for (final child in children)
            _ChildOverviewCard(
              child: child,
              hasAttention: dashboard.alerts.any(
          ...
          // Phase 6A: the previous "أطفال يحتاجون متابعة" and
          // "الإشعارات والتنبيهات" sections showed the same backend alerts
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // FittedBox keeps long textual values (dates, "بلا بيانات بعد")
          // readable on ~360px-wide devices instead of clipping them.
          FittedBox(
...
/// Phase 6A: unified follow-up section. Backend-driven alerts only, shown
/// once, sorted by severity (HIGH first) and grouped per child.
class _FollowUpSection extends StatelessWidget {
  final List<ParentAlert> alerts;

  const _FollowUpSection({required this.alerts});
  ...
  Widget build(BuildContext context) {
    ...
    final sorted = [...alerts]
      ..sort((a, b) {
        final aHigh = a.severity == 'HIGH' ? 0 : 1;
        final bHigh = b.severity == 'HIGH' ? 0 : 1;
              ...
              if (studentName.isNotEmpty)
                Row(
                  children: [
                    ...
                    _StatusBadge(
                      label: isHigh ? 'عاجل' : 'للمتابعة',
                      color: color,
                    ),
                  ],
                ),
              if (studentName.isNotEmpty) const SizedBox(height: 3),
              Text(
                alert.message,
                maxLines: 3,
                overflow: TextOverflow.ellipsis,
