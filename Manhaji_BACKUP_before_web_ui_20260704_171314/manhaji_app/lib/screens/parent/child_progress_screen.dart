  ...
  void didChangeDependencies() {
    ...
    if (!_loaded) {
      ...
      if (args is ChildProgressArgs) {
        ...
        context.read<ParentProvider>().loadChildDetail(args.studentId);
      } else {
        _invalidArgs = true;
      }
      _loaded = true;
...
class _ChildDetailsContent extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final strengths = student.subjectBreakdown
        .where((subject) => subject.averageMastery >= 75)
        .toList();
    final weaknesses = student.subjectBreakdown
        .where((subject) => subject.averageMastery < 65)
        .toList();
    ...
    return ListView(
      ...
      children: [
        ...
        const SizedBox(height: 20),
        const _SectionTitle(title: 'قائمة إنجاز الدروس'),
        const SizedBox(height: 10),
        _LessonCompletionList(subjects: student.subjectBreakdown),
        const SizedBox(height: 20),
        const _SectionTitle(title: 'أداء الاختبارات الأخيرة'),
        const SizedBox(height: 10),
        _QuizPerformanceSummary(student: student),
        const SizedBox(height: 20),
        const _SectionTitle(title: 'نقاط القوة'),
        const SizedBox(height: 10),
        strengths.isEmpty
            ? const _EmptyInfoCard(
                icon: Icons.workspace_premium_rounded,
                title: 'لم تظهر نقاط قوة واضحة بعد',
                message: 'سيتم تحديث هذه القائمة بعد توفر بيانات مواد أكثر.',
              )
            : _SkillList(
                icon: Icons.workspace_premium_rounded,
        ...
        const SizedBox(height: 20),
        const _SectionTitle(title: 'نقاط تحتاج مراجعة'),
        const SizedBox(height: 10),
        weaknesses.isEmpty
            ? const _EmptyInfoCard(
                icon: Icons.flag_rounded,
                title: 'لا توجد نقاط ضعف واضحة حالياً',
                message: 'استمر في متابعة النشاط للحفاظ على هذا المستوى.',
              )
            : _SkillList(
                icon: Icons.flag_rounded,
        ...
        const SizedBox(height: 20),
        const _SectionTitle(title: 'توصيات لولي الأمر'),
        const SizedBox(height: 10),
...
class _LessonCompletionList extends StatelessWidget {
  ...
  const _LessonCompletionList({required this.subjects});
  ...
  Widget build(BuildContext context) {
    if (subjects.isEmpty) {
      return const _EmptyInfoCard(
        ...
        title: 'لا توجد قائمة دروس بعد',
        message: 'سيظهر إنجاز الدروس عندما يرسل الخادم تفاصيل المواد.',
      );
    }
      ...
      child: Column(
        children: [
          for (var i = 0; i < subjects.length; i++) ...[
            _LessonRow(subject: subjects[i]),
            if (i != subjects.length - 1) const Divider(height: 22),
          ],
        ],
      ),
    );
...
class _LessonRow extends StatelessWidget {
  ...
  const _LessonRow({required this.subject});
      ...
      children: [
        ...
        Text(
          ...
          style: TextStyle(
            ...
            fontSize: 12,
            fontWeight: FontWeight.w800,
            color: remaining == 0 ? AppTheme.primaryGreen : AppTheme.textGray,
          ),
        ),
      ],
    );
          ...
          child: Column(
            ...
            children: [
              ...
              if (attempt.subjectName != null || attempt.lessonTitle != null)
                Text(
                  [attempt.subjectName, attempt.lessonTitle]
                      .whereType<String>()
                      .join(' — '),
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
  ...
  Widget build(BuildContext context) {
    if (subjects.isEmpty) {
      return _EmptyInfoCard(
        icon: icon,
        title: emptyMessage,
        message: 'سيتم تحديثها تلقائياً عند توفر بيانات جديدة.',
      );
    }
          ...
          for (var i = 0; i < subjects.length; i++) ...[
            Row(
              children: [
                ...
                Text(
                  '${subjects[i].averageMastery.toStringAsFixed(0)}%',
                  style: TextStyle(
                    fontFamily: 'Cairo',
                    fontSize: 13,
                    fontWeight: FontWeight.w900,
  ...
  Widget build(BuildContext context) {
    if (recommendations.isEmpty) {
      return const _EmptyInfoCard(
        ...
        message: 'طفلك يسير بشكل جيد! ستظهر توصيات هنا عند الحاجة لمتابعة إضافية.',
      );
    }
